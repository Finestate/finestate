  ...
  return (
    <div className="w-full">
      <div className="w-full border-2 border-neutral-400 bg-white shadow-sm overflow-hidden">
        <div className="flex h-[18px] items-center gap-2 border-b border-neutral-400 pl-2 pr-6" style={{ backgroundColor: BAR_BG }}>
          <span className={`flex-1 ${head}`}>{TITLE}</span>
          <span className={`w-32 shrink-0 whitespace-nowrap text-left ${head}`}>Monthly cost</span>
        </div>

        <div>
          {COSTS.map((c, i) => (
            <div key={c.item} className={`flex h-[21px] items-center gap-2 pl-2 pr-6 ${i === 0 ? "" : "border-t border-neutral-300"}`}>
        ...
        <div className="flex h-[18px] items-center gap-2 border-t border-neutral-400 pl-2 pr-6">
